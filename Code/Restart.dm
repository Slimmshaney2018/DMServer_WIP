	...
	if(!ASK)
		...
		if(client)
			...
			switch(Race)
				...
				if("Lizardman","Frogman","Naga") LOC=LocateValidLocation(X=175,XX=365,Y=675,YY=785)
			...
			if(TYPE) for(var/mob/Monsters/M in MakeMany(TYPE,NUMBER,LOC,1))
				...
				if(NUMBER>1) switch(CURNUM)
					if(1)
						M.x-=1
						switch(M.Race)
							if("Frogman","Lizardman","Naga") M.ForcePickUpItem(new/obj/Items/woods/wood(loc))
				...
				switch(M.Race) if("Human","Dwarf","Goblin","Orc","Lizardman","Frogman","Kobold","Ratman","Naga") M.ForcePickUpItem(new/obj/Items/Furniture/Beds/SleepingBag)
				...
				switch(M.Race)
					...
					if("Lizardman","Frogman","Kobold","Ratman","Naga")
			...
			switch(Race)
				...
				if("Naga") alert(src,"Driven from your home in the swarm,You arrive in [WorldName] Life is what you will Do with it as you please")
